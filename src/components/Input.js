...
export default function Input() {
  ...
  const stil = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: inputValue.length > 10 ? 'crimson' : 'royalblue', /* ADIM 2 */
  };

  return (
    <div className='widget-input container'>
      <h2>Input</h2>
      <div id='output' style={stil}>{inputValue.toUpperCase()}</div> {/* ADIM 3 */}
      <div>
		<input id='input' type='text' onChange={inputuDeğiştir} value={inputValue}/> {/* ADIM 6 */}
